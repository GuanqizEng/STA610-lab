...
- `ppm`: response variable, price of morphine per milligram, numeric value. (Notation in code: `ppm`) 
- `city`: cities, where the transaction took place, 1654 unique factors. (Notation in code: `City_final`)
- `state`: states, where the transaction took place, 56 unique factors. (Notation in code: `state`)
- `region`: regions, where the transaction took place, 5 unique factors. (Notation in code: `USA_region`)
- `primary reason`: reasons for purchasing morphine, 11 unique factors. (Notation in code: `Primary_Reason`)
- `source`: sources of such transactions, 8 unique values. (Notation in code: `Source_class`)
- `days ellapsed`: number of days passed since 01/01/2010, which is the time when the data collection process started, discrete numerical value. (Notation in code: `Days_since_010110`)
- `dosage strength`: dosage strength of the purchased morphine, discrete numeric value, ranging from 1 to 200. (Notation in code: `mgstr`)
- `bulk`: indicator for purchased 10+ units, factor with levels 0 and 1. (Notation in code: `Bulk`)
...
```{r, echo = F, fig.asp=0.55, fig.cap="log ppm versus different location grouping variables."}
set.seed(100)
sample_city <- sample(unique(data$City_final), 8, replace=FALSE)
city_group <- ggplot(data[is.element(data$City_final, sample_city),], 
       aes(x=City_final, y=log(ppm), fill=City_final)) + 
  geom_boxplot(outlier.size = 0.3) + 
  labs(y="log(ppm)", x="city") + 
  scale_x_discrete(labels=c("Eureka", "Foley", "Morgantown", "Naples", "Unknown", "Simi Valley", "Wadesboro", "Yreka")) + 
  theme_classic() + 
...
set.seed(100)
sample_state <- sample(unique(data$state), 10, replace=FALSE)
state_group <- ggplot(data[is.element(data$state, sample_state),], 
       aes(x=state, y=log(ppm), fill=state)) + 
  geom_boxplot(outlier.size = 0.3) + 
  labs(y="log(ppm)", x="state") + theme_classic() + 
  theme(legend.position="none", 
        axis.text.x= element_text(angle=90, hjust=1, size=5),
        axis.title.y = element_blank())

region_group <- ggplot(data, aes(x=USA_region, y=log(ppm), fill=USA_region)) + 
  geom_boxplot(outlier.size = 0.3) + 
  labs(y="log(ppm)", x="region") + theme_classic() + 
  theme(legend.position="none", 
        axis.text.x= element_text(angle=90, hjust=1, size=5),
        axis.title.y = element_blank())

city_group + state_group + region_group 
```
...
```{r fig.asp=0.4, fig.cap="Distribution of State Random Effects"}
par(mfrow =c(3,1))
...
```{r, warning=F, message = F, fig.cap="log ppm versus dosage strength overall, and log ppm by time in different states.", fig.height=3}
mgstr_plot <- ggplot(data, aes(x=mgstr, y=log(ppm))) + 
  geom_point(size=0.3) + 
  labs(x="dosage strength", y="log(ppm)") + 
  theme_bw(base_size=8) 
set.seed(108)
sample_state <- sample(unique(data$state), 6, replace=FALSE)
state_plot <- ggplot(data[is.element(data$state, sample_state), ], aes(x=Days_since_010110, y=log(ppm), color=state)) +
  geom_point(size=0.3) + 
  geom_smooth(method="lm", fill=NA) +
  labs(x = "Days since 2010/01/01", y = "log(ppm)") + 
  theme_bw(base_size=8)

mgstr_plot + state_plot 
```
...
We also considered adding a random slope by dosage strength by state.**(See Appendix Fig.14)**
...
Table 2. The BIC scores of the models with specified random effects and fixed effects. For convenience, we used `mgstr` to represent `dosage strength` here.   

|   Name    |         DF         |  BIC Score |   Random Effects                         | Fixed Effect                       | 
| :-------: | :-----------------:|:----------:|:---------------------------------------: |:---------------------------------: |
|    base   |         3          |   24124    | `state`                                  | None                               |
|    m1     |         4          |   23105    | `state`                                  | `mgstr`                            |
|    m2     |         4          |   23100    | `state` + `mgstr` (by `state`)           | `mgstr`                            |
|    m3     |         5          |   23098    | `state`                                  | `mgstr` + `bulk`                   |
|    m4     |         6          |   23105    | `state`                                  | `mgstr` * `bulk`                   |
|    m5     |         12         |   23134    | `state`                                  | `mgstr` + `source` + `bulk`        |
|    m6     |         47         |   23442    | `state` + `source` (by `state`)          | `mgstr` + `source` + `bulk`        |
|    m7     |         19         |   23182    | `state`                                  | `mgstr` + `source` * `bulk`        |
|    m8     |         15         |   23182    | `state`                                  | `mgstr` + `primary reason` + `bulk`|
|    m9     |         80         |   23140    | `state` + `primary reason` (by `state`)  | `mgstr` + `primary reason` + `bulk`|
|    m10    |         6          |   23104    | `state`                                  | `mgstr` + `bulk` + `days ellapsed` |
|    m11    |         8          |   23117    | `state` + `days ellapsed` (by `state`)   | `mgstr` + `bulk` + `days ellapsed` |

First, from the **(mgstr EDA graph)**, we could see that there was a slightly negative relationship between the `log(ppm)` and the `dosage strength` variable. Since the `ppm` was log-transformed, a slightly negative coefficient between `log(ppm)` and `dosage strength` might be enlarged after exponentiation. Therefore, we considered the `dosage strength` as a quite important predictor and added it to the model. By comparing with the base model, the model with the addition of `dosage strength` as a fixed effect had much lower BIC score (Table 2. m1 vs base model). Therefore, we decided to include `dosage strength` in the model. Now, the model contained random effects for `state` and fixed effects for `dosage strength`.  
...
Table 3. Model Coefficients Summary.  

|    Coefficient     |  Fixed Effect |   Lower Bd |  Upper Bd |
|      :---:         |     :---:     |    :---:   |   :---:   |
| `Intercept`        |    -0.590     |    -0.639  |  -0.541   |
| `Dosage Strength`  |    -0.010     |    -0.010  |  -0.009   |
| `Bulk`             |    -0.103     |    -0.154  |  -0.052   |
...
```{r, warning=FALSE, echo=FALSE, fig.align="center", fig.cap="Random effect by state from the model.", fig.asp=0.4}
sjPlot::plot_model(model, type="re") + theme_sjplot(base_size=6)
```
...
```{r fig.cap="Diagnostic Plots, residual plots and normal qq plot.", fig.height=2, fig.asp=0.4, fig.align='center' }
temp <- data.frame("residuals"     = resid(model), 
                   "fitted_values" = fitted(model))
resid_plot <- ggplot(temp, aes(fitted_values, residuals)) +
  geom_point(size=0.8, na.rm = TRUE) + 
  theme_bw() + 
  labs(x="fitted values", y="residuals")

temp <- data.frame("resid" = resid(model)) 
inter <- mean(resid(model))
sddev <- sd(resid(model))
qq_plot <- ggplot(temp) + 
  geom_qq(aes(sample=resid)) + 
  geom_abline(intercept=inter, slope=sddev, 
...
Below are the relevant figures and tables to the project. 

```{r, fig.cap="log ppm differs by state"}
...
```{r, warning=FALSE, echo=FALSE, fig.cap="Diagnostic Plots ctn.", fig.width=2.5, fig.height=2.5}
par(mfrow = c(1,1))
