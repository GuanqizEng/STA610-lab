...
```{r, echo=F, fig.asp=0.4, fig.align='center'}
p1 <- ggplot(data) +
...
However, by only looking at the relationship between this variable and the log ppm, we do not observe obvious linear association.(See Appendix Fig.1)
...
```{r, warning=F, message = F, echo = F, fig.asp=0.6}
set.seed(108)
...
We would not consider a random slope by dosage strength by state.(See Appendix Fig.8)

##### Primary Reason (See Appendix Fig.2 and Fig.3)
...
##### Source (See Appendix Fig.4 and Fig.5)
...
##### Bulk (See Appendix Fig.6 and Fig.7)
...
```{r, echo = F, fig.asp=0.4, fig.cap="Relationship between log ppm Days Elasped"}
